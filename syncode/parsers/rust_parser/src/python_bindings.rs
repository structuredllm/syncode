...
impl RustLexer {
    ...
    fn initialize(
        ...
        for (name, pattern_obj, priority) in terminal_defs {
            ...
            let pattern_type: String = pattern_dict
                .get_item("type")
                .ok_or_else(|| {
                    PyErr::new::<pyo3::exceptions::PyValueError, _>(
                        "Missing 'type' in pattern dictionary",
                    )
                })?
                .extract()?;

            let pattern_value: String = pattern_dict
                .get_item("value")
                .ok_or_else(|| {
                    PyErr::new::<pyo3::exceptions::PyValueError, _>(
                        "Missing 'value' in pattern dictionary",
                    )
                })?
                .extract()?;
        ...
        match self.lexer.initialize(terminals, ignore_types) {
            ...
            Err(LexerError::RegexError(e)) => {
                Err(PyErr::new::<pyo3::exceptions::PyRuntimeError, _>(format!(
                    "Failed to compile regular expressions: {}",
                    e
                )))
            }
            ...
            Err(e) => {
                ...
                match e {
                    ParserError::UnexpectedToken {
                        ...
                        let err_dict =
                            error_dict.get_item("error").unwrap().downcast::<PyDict>()?;
